############################################################
# LLM-based coding with full article content
# Web scraping + GPT-based content coding
############################################################
...
scarica_contenuto_url <- function(url) {
  if (is.na(url) || trimws(as.character(url)) == "") return(list(successo = FALSE, testo = "", errore = "url_missing"))
  cat("Scarico:", url, "\n")
  tryCatch({
    response <- GET(url,
    ...
    if (sc != 200) return(list(successo = FALSE, testo = "", errore = paste0("HTTP ", sc)))
    ...
    meta_desc <- doc %>% html_elements("meta[name='description'], meta[property='og:description']") %>% html_attr("content") %>% paste(collapse = " ")
    ...
    for (selettore in corpo_selettori) {
      ...
      if (length(elementi) > 0) { corpo <- elementi %>% html_text2() %>% paste(collapse = " "); break }
    }
    if (nchar(corpo) < 100) corpo <- doc %>% html_elements("p") %>% html_text2() %>% paste(collapse = " ")
    testo_completo <- paste(titolo, meta_desc, corpo, sep = "\n\n")
    testo_pulito <- gsub("\\s+", " ", testo_completo) %>% trimws()
    if (nchar(testo_pulito) > 8000) testo_pulito <- substr(testo_pulito, 1, 8000)
    list(successo = TRUE, testo = testo_pulito, errore = "")
  }, error = function(e) list(successo = FALSE, testo = "", errore = as.character(e$message)))
}

############################################################
# GPT call with full article content
############################################################

call_gpt_con_contenuto <- function(testo_articolo) {
  if (is.na(OPENAI_API_KEY) || OPENAI_API_KEY == "") stop("OPENAI_API_KEY mancante")
  prompt <- paste0(
  ...
  res <- tryCatch({
    response <- POST(
      ...
      add_headers(Authorization = paste("Bearer", OPENAI_API_KEY), "Content-Type" = "application/json"),
      encode = "json",
      body = list(model = "gpt-4o-mini", temperature = 0, messages = list(list(role = "user", content = prompt))),
      timeout(60)
    )
    sc <- status_code(response)
  ...
  }, error = function(e) list(error = TRUE, message = as.character(e$message)))
  return(res)
}
...
############################################################
cat("=== CODIFICA CON CONTENUTO COMPLETO DELLE URL ===\n")
cat("Carico i dati...\n")
data <- read_xlsx(INPUT_FILE)

if (is.null(N_RIGHE) || !is.numeric(N_RIGHE) || N_RIGHE <= 0) N_RIGHE <- nrow(data)
N_RIGHE <- min(as.integer(N_RIGHE), nrow(data))
cat("Processando righe 1..", N_RIGHE, " su ", nrow(data), " totali\n", sep = "")

output_file <- file.path(dirname(INPUT_FILE), "risultati_codifica_con_contenuto.xlsx")
if (!dir.exists(dirname(output_file))) dir.create(dirname(output_file), recursive = TRUE, showWarnings = FALSE)

expected_gold <- c("societal_actors_gold","decision_authorities_gold","policy_plan_gold",
...
for (c in expected_gold) if (!c %in% names(data)) data[[c]] <- NA_integer_
...
safe_call_gpt_with_retries <- function(testo, attempts = max_attempts_gpt) {
  for (a in seq_len(attempts)) {
    ...
    res <- call_gpt_con_contenuto(testo)
...
for (i in seq_len(N_RIGHE)) {
  ...
  if (isTRUE(risultato_download$successo) && nchar(risultato_download$testo) > 50) {
    cat("Contenuto scaricato:", nchar(risultato_download$testo), "caratteri. Chiamo GPT...\n")
  ...
  tryCatch({ openxlsx::write.xlsx(data, output_file, overwrite = TRUE) }, error = function(e) cat("Warning: errore salvataggio parziale:", e$message, "\n"))
...
}, error = function(e) cat("Errore salvataggio finale:", e$message, "\n"))
...
if (length(good_idx) > 0) {
  esempi <- data[good_idx[1:min(3, length(good_idx))], c("headline", expected_gold, "error_message"), drop = FALSE]
  print(esempi)
} else {
...
output_csv <- file.path(dirname(output_file), "risultati_codifica_con_contenuto.csv")

write.csv(data, output_csv, row.names = FALSE, na = "")

cat("\n--- SALVATAGGIO COMPLETATO ---\n")
cat("File XLSX (principale):", output_file, "\n")
cat("File CSV (fallback):   ", output_csv, "\n")
cat("Righe salvate:         ", nrow(data), "\n")